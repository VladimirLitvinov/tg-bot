...
async def err_city_input(message: Message):
...
async def exit_date(callback_query: CallbackQuery, callback_data: CallbackData, state: FSMContext):
    calendar = SimpleCalendar(
...
async def adults_quantity(callback_query: CallbackQuery, callback_data: CallbackData, state: FSMContext):
    calendar = SimpleCalendar(
        locale=await get_user_locale(callback_query.from_user), show_alerts=True
    )
    selected, date = await calendar.process_selection(callback_query, callback_data)
...
async def get_adults(message: Message, state: FSMContext):
...
async def get_adults_err(message: Message):
...
async def more_results(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
